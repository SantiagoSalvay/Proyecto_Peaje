...
class RegistroTelepase(models.Model):
    ...
    fecha_hora_de_emision = models.DateTimeField()
    importe_cobrado = models.FloatField()
    codigo_telepase = models.CharField(max_length=255, null=True, blank=True)
    usuario = models.CharField(max_length=255, null=True, blank=True)
    direccion = models.CharField(max_length=255, null=True, blank=True)
    patente = models.CharField(max_length=255, null=True, blank=True)
    tarjeta_debito = models.IntegerField(null=True, blank=True)
    id_historial_tarifas = models.ForeignKey(HistorialTarifas, on_delete=models.CASCADE)
...
class NoPagados(models.Model):
    ...
    def __str__(self):
        return f"No Pagado {self.id_no_pagado} - {self.nombre_conductor} {self.apellido_conductor}"
...
class Pasada(models.Model):
    ...
    def __str__(self):
        return f"Pasada {self.id_pasada} - Vehículo {self.id_tipo_vehiculo}"
